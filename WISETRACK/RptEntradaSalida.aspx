<%@ Page Title="" Language="C#" MasterPageFile="~/SitePrincipal.Master" AutoEventWireup="true" CodeBehind="RptEntradaSalida.aspx.cs" Inherits="WISETRACK.RptEntradaSalida" %>

<%@ Register Assembly="Telerik.Web.UI" Namespace="Telerik.Web.UI" TagPrefix="telerik" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <script>
        function CheckItemGeocerca(sender, eventArgs) {
            ...
            sender.set_text("Seleccionar Geocercas");
        }
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
        ...
        <div class="row-fluid">
            <div class="span3">
                ...
                <div class="panel panel-primary">
                    <div class="panel-heading">Reporte de Entrada/Salida</div>
                    ...
                    <div class="panel-body">
                        ...
                        <div class="form-inline">
                            <div class="form-group">
                                <telerik:RadComboBox ID="rcbEstado" runat="server" DropDownCssClass="dropdown" AllowCustomText="false" EmptyMessage="" Width="160px" CssClass="dropdown">
                                    <Items>
                                        <telerik:RadComboBoxItem Value="2" Text="ENTRADA/SALIDA" />
                                    </Items>
                                </telerik:RadComboBox>
                            </div>
                        </div>
                        ...
                        <div class="form-inline">
                            <div class="form-group">
                                <asp:Label ID="lblGeocerca" runat="server" Text="<b>Geocerca</b>" Font-Size="Small"></asp:Label>
                            </div>
                        </div>
                        <div class="form-inline">
                                ...
                                <div class="col-sm-9 table-responsive" style="padding-left: 0; margin-left: 5%; width: 92%; margin-right: 3%; padding-right: 0px; right: 0%; top: 0px;">
                                    <asp:GridView ID="gdvGeocercas" runat="server" AutoGenerateColumns="False"
                                        ...
                                        <Columns>
                                            ...
                                            <asp:TemplateField>
                                                <HeaderTemplate>
                                                    <asp:CheckBox ID="CheckAllZona" runat="server" Font-Size="Smaller" onclick="CheckAllZona(this);" />
                                                </HeaderTemplate>
                                                <ItemTemplate>
                                                    <asp:CheckBox ID="SelecTipoZona" runat="server" Font-Size="Smaller" Visible="true" onclick="UnCheckAllZona(this);" />
                                                </ItemTemplate>
                                            </asp:TemplateField>

                                            <asp:BoundField DataField="CodigoGeo" HeaderText="CodigoGeo" ReadOnly="true" SortExpression="CodigoGeo"
                        ...
                        <div class="form-inline">
                            <div class="form-group">
                                <%--<telerik:RadComboBox ID="cboplaca" runat="server" AllowCustomText="true" 
                                    OnItemDataBound="cboplaca_ItemDataBound" CheckBoxes="true"
                                    EnableCheckAllItemsCheckBox="false" 
                                    OnClientItemChecked="CheckItemVehiculo">
    ...
    <script>
        ...
        function CheckAllEmp(Checkbox) {
            var GridVwHeaderChckbox = document.getElementById("<%=gdvGeocercas.ClientID %>");
            for (i = 1; i < GridVwHeaderChckbox.rows.length; i++) {
                GridVwHeaderChckbox.rows[i].cells[3].getElementsByTagName("INPUT")[0].checked = Checkbox.checked;
            }
        }

        function CheckAllZona(Checkbox) {
            var gdvTiposAlarma = document.getElementById("<%= gdvGeocercas.ClientID %>");
        ...
        function UnCheckAllZona(Checkbox) {
            var gdvTiposAlarma = document.getElementById("<%= gdvGeocercas.ClientID %>");
            var ckbAllTipoAlarma = gdvTiposAlarma.rows[1].cells[0].getElementsByTagName("INPUT")[0];
        ...
        $(document).ready(function () {
            ...
            <%--$("#<%= gdvGeocercas.ClientID %> input[id*='chkHeader']").click(function () {
                var ies = $(this).is('checked');
                var ck = this.checked;
                if (ck)
                    //Checkd All Child checkbox  
                    $("#<%= gdvGeocercas.ClientID %> input[id*='chkChild']").attr('checked', this.checked);
                else
                    //UnCheckd All Child checkbox  
                    $("#<%= gdvGeocercas.ClientID %> input[id*='chkChild']").attr('checked', this.checked);
            });
            // Header checkbox checked or unchecked inside grid row checkboxes   
            $("#<%=gdvGeocercas.ClientID%> input[id*='chkChild']checkbox").click(function () {
                //Get All Checkbox inside grid  
                var GetAllCheckboxes = $("#<%=gdvGeocercas.ClientID%> input[id*='chkChild']checkbox").size();
                //Get number of checked checkboxes inside grid   
                var MarkcheckedCheckboxes = $("#<%=gdvGeocercas.ClientID%> input[id*='chkChild']checkboxchecked").size();
                //Check / Uncheck top checkbox if all the checked boxes in list are checked  
                $("#<%=gdvGeocercas.ClientID%> input[id*='chkHeader']checkbox").attr('checked', GetAllCheckboxes == MarkcheckedCheckboxes);
            });--%>
        });

using System;
using System.Web;
using System.Web.UI;
using Microsoft.AspNet.Identity;
using Microsoft.AspNet.Identity.Owin;
using Owin;
using WISETRACK.Models;
using System.Web.Services;
using System.Collections.Generic;
using WISETRACK.Datos.Auxiliar;
using Newtonsoft.Json;
using System.Web.Script.Services;


namespace WISETRACK.Account
{
    public partial class ForgotPassword : Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
        }

		[WebMethod]
		[ScriptMethod(ResponseFormat = ResponseFormat.Json)]
		public static string setEnviar(string email = "")
		{
			var mensaje = new List<KeyValuePair<string, string>>();
			// Validar la dirección de correo electrónico del usuario
			//var mg = HttpContext.Current.GetOwinContext().GetUserManager<ApplicationUserManager>();
			//var manager = Context.GetOwinContext().GetUserManager<ApplicationUserManager>();
			
			var manager = HttpContext.Current.GetOwinContext().GetUserManager<ApplicationUserManager>();
			//ApplicationUser user = manager.FindByName(Email.Text);
			ApplicationUser user = manager.FindByEmail(email);
			if (user == null || !manager.IsEmailConfirmed(user.Id))
			{
			//	FailureText.Text = "El usuario no existe o no se ha confirmado.";
			//	ErrorMessage.Visible = true;
			//	return;
				mensaje.Add(Utils.mensaje("ERROR", "Esta correo no esta asociado con ningun usuario, conectese con su proveedor..."));
				return JsonConvert.SerializeObject(mensaje);
			}
			// Para obtener más información sobre cómo habilitar la confirmación de cuentas y el restablecimiento de contraseña, visite http://go.microsoft.com/fwlink/?LinkID=320771
			// Enviar correo electrónico con el código y la dirección de redireccionamiento a la página de restablecimiento de contraseña
			string code = manager.GeneratePasswordResetToken(user.Id);
			string callbackUrl = IdentityHelper.GetResetPasswordRedirectUrl(code,HttpContext.Current.Request);
			manager.SendEmail(user.Id, "Restablecer contraseña", "Para restablecer la contraseña, haga clic <a href=\"" + callbackUrl + "\">aquí</a>.");
			mensaje.Add(Utils.mensaje("OK", "Coreo electronico esta vinculado con el siguiente usuario: ("+user.UserName+")"));
			//loginForm.Visible = false;
			//DisplayEmail.Visible = true;

			return JsonConvert.SerializeObject(mensaje); 
		}
		
        protected void Forgot(object sender, EventArgs e)
        {
            if (IsValid)
            {
                // Validar la dirección de correo electrónico del usuario
                var manager = Context.GetOwinContext().GetUserManager<ApplicationUserManager>();
                //ApplicationUser user = manager.FindByName(Email.Text);
                ApplicationUser user = manager.FindByEmail(Email.Text);
                if (user == null || !manager.IsEmailConfirmed(user.Id))
                {
                    FailureText.Text = "El usuario no existe o no se ha confirmado.";
                    ErrorMessage.Visible = true;
                    return;
                }
                // Para obtener más información sobre cómo habilitar la confirmación de cuentas y el restablecimiento de contraseña, visite http://go.microsoft.com/fwlink/?LinkID=320771
                // Enviar correo electrónico con el código y la dirección de redireccionamiento a la página de restablecimiento de contraseña
                string code = manager.GeneratePasswordResetToken(user.Id);
                string callbackUrl = IdentityHelper.GetResetPasswordRedirectUrl(code, Request);
                manager.SendEmail(user.Id, "Restablecer contraseña", "Para restablecer la contraseña, haga clic <a href=\"" + callbackUrl + "\">aquí</a>.");
				//loginForm.Visible = false;
				//DisplayEmail.Visible = true;
            }
        }
    }
}